...
#define LOG_TAG "sound_trigger_hw_exynos"
/*#define LOG_NDEBUG 0*/
...
#define EXYNOS_MIXER_VAD	0
#define EXYNOS_CTRL_DSP	"VAD Mode"
#define UEVENT_MSG_LEN		1024

#define EXYNOS_VAD_DEV	"/dev/snd/hwC0D0"

#define EXYNOS_STREAMING_DELAY_USEC	1000
#define EXYNOS_STREAMING_READ_RETRY_ATTEMPTS	30
#define EXYNOS_STREAMING_BUFFER_SIZE	(16 * 1024)
...
struct EXYNOS_sound_trigger_device {
...
static struct EXYNOS_sound_trigger_device g_stdev = { .lock = PTHREAD_MUTEX_INITIALIZER };

static void stdev_dsp_set_power(struct EXYNOS_sound_trigger_device *stdev,
                                int val)
{
...
static int stdev_init_mixer(struct EXYNOS_sound_trigger_device *stdev)
{
    int ret = -1;

    stdev->vad_fd = open(EXYNOS_VAD_DEV, O_RDWR);
    if (stdev->vad_fd < 0) {
        ALOGE("Error opening vad device");
        return ret;
    }

    stdev->mixer = mixer_open(EXYNOS_MIXER_VAD);
    if (!stdev->mixer)
        goto err;

    stdev->ctl_dsp = mixer_get_ctl_by_name(stdev->mixer, EXYNOS_CTRL_DSP);
    if (!stdev->ctl_dsp)
        goto err;
...
static void stdev_close_term_sock(struct EXYNOS_sound_trigger_device *stdev)
{
...
static void stdev_close_mixer(struct EXYNOS_sound_trigger_device *stdev)
{
    if (stdev) {
...
static int vad_load_sound_model(struct EXYNOS_sound_trigger_device *stdev,
                                char *buf, size_t len)
{
...
static char *sound_trigger_event_alloc(struct EXYNOS_sound_trigger_device *
                                       stdev)
{
    char *data;
...
static int fetch_streaming_buffer(struct EXYNOS_sound_trigger_device * stdev)
{
    struct rt_codec_cmd cmd;
    int ret = 0;
    int i;

    cmd.number = EXYNOS_STREAMING_BUFFER_SIZE / sizeof(int);
    cmd.buf = (int*) stdev->streaming_buf;

    ALOGV("%s: Fetching bytes", __func__);
    for (i = 0; i < EXYNOS_STREAMING_READ_RETRY_ATTEMPTS; i++) {
        ret = ioctl(stdev->vad_fd, RT_READ_CODEC_DSP_IOCTL, &cmd);
        if (ret == 0) {
            usleep(EXYNOS_STREAMING_DELAY_USEC);
        } else if (ret < 0) {
...
static void *callback_thread_loop(void *context)
{
    ...
    struct EXYNOS_sound_trigger_device *stdev =
               (struct EXYNOS_sound_trigger_device *)context;
    struct pollfd fds[2];
    int exit_sockets[2];
    int err = 0;
    int i, n;
...
{
    struct EXYNOS_sound_trigger_device *stdev =
                               (struct EXYNOS_sound_trigger_device *)dev;
...
{
    struct EXYNOS_sound_trigger_device *stdev = &g_stdev;
    int ret = 0;
...
{
    struct EXYNOS_sound_trigger_device *stdev =
                                (struct EXYNOS_sound_trigger_device *)device;
    int ret = 0;
                      ...
                      hw_device_t **device)
{
    ...
    stdev->streaming_buf = malloc(EXYNOS_STREAMING_BUFFER_SIZE);
